...
import styles from './viewPendingRequests.module.scss';
import { Loader } from '../../../../../../../components/loader/loader';
import { Button, Card, Col, Container, Image, Modal, Row } from 'react-bootstrap';
import { editorGetAllPendingWorkshopProposals, editorGetPendingWorkshopProposal } from '../../../../../../../services/util/editor/workshops';
import { Document, Page, pdfjs } from 'react-pdf';
pdfjs.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjs.version}/pdf.worker.js`

function ViewPendingWorkshopRequests() {
    let [workshop_request_list, set_workshop_request_list] = useState([]);
    ...
    async function LoadWorkshopProposal(id) {
        set_modal_show(true);
        set_single_item_loading(true);
        let respond = await editorGetPendingWorkshopProposal(id);
        if (respond.success) {
    ...
    return (
        <div>
            {is_page_loading ?
                ...
                <Container fluid>
                    <Row>
                        {(workshop_request_list && (workshop_request_list).length > 0) ?
                            (workshop_request_list).map((values, idx) => (
                                <Col sm={3} key={idx}>
                                    <Card className={styles.workshopProposalCard} onClick={() => LoadWorkshopProposal(values.workshop_conductor_id)}>
                            ...
                            <Col className={styles.MainMessageComponent}>
                                <div className={styles.innerMessageComponent}>
                                    ...
                                    <p className={styles.emptyMessage}>No workshop requests</p>
                                </div>
                            </Col>
                        }
                    </Row>
            ...
            <Modal show={modal_show} onHide={closeModal} centered size="lg">
                <Modal.Header closeButton>
                    <Modal.Title>Workshop Request</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                            ...
                            <Col className='mb-4'>
                                <Card className={styles.actionCard}>
                                    <Card.Body className={styles.actionCardBody}>
                                        <Row>
                                            <Col xs={4} lg={2}>
                                                <Image src={single_request.workshop_conductor_image ? single_request.workshop_conductor_image : `${process.env.REACT_APP_BASE_URL}/images/dummy-avatar.png`} className={styles.workshopConductorImage2} />
                                            </Col>
                                            <Col className='align-self-center'>
                                                <p className={styles.workshopConductorName}>{single_request.workshop_conductor_name}</p>
                                                <p className={styles.workshopConductorEmail}>{single_request.workshop_conductor_email_address}</p>
                                            </Col>
                                        </Row>
                                    </Card.Body>
                                </Card>
                                ...
                                <Card className={styles.cvViewCard}>
                                    <Card.Header className={styles.cardHeader}>
                                        <div className={styles.headerDetails}>
                                            <p className={styles.accordianHeaderTopic}><b>CV of</b>&nbsp;&nbsp;{single_request.workshop_conductor_name}</p>
...
export default ViewPendingWorkshopRequests
